...
import { escapeHtml, isHexColor } from "../../utils/html.js";
import { DEFAULT_ACCENT, MAP_CENTER, MAP_ZOOM, TILE_LAYER } from "../../app/config.js";
 ...
 *
 * Un valor mal escrito —por ejemplo `"#7C8B4A;"`— se descarta aquí en lugar de
 * llegar al CSS, donde fallaba en silencio y solo en algunos sitios.
 *
 * @param {Place} place
 * @returns {string}
 */
export function accentOf(place) {
 ...
 * }} options
 */
export function createMapView({ container, leaflet: L, onSelect }) {
    ...
    setPlaces(places) {
      ...
      for (const place of places) {
        const icon = L.divIcon({
          className: "",
          // `escapeHtml` protege el atributo de estilo y el contenido: el emoji
          // y el color vienen de datos, no del código.
          html:
