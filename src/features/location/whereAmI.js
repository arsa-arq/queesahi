 ...
 * de modo que la lógica del flujo se puede seguir —y más adelante probar— sin
 * depender de un mapa real.
 */

import { nearest } from "../../services/geoService.js";
import { formatDistance } from "../../services/geoService.js";
import { getCurrentPosition, getLastKnownPosition } from "../../services/locationService.js";
import { FAR_AWAY_METERS } from "../../app/config.js";

/** @typedef {import("../../types/place.js").Place} Place */
/** @typedef {import("../../types/place.js").GeoPoint} GeoPoint */
/** @typedef {import("../../services/locationService.js").LocationErrorCode} LocationErrorCode */

/**
 * Mensajes de error orientados a la acción: cada uno dice qué puede hacer la
 * persona a continuación.
 * @type {Record<LocationErrorCode, string>}
 */
 ...
 */
export function createLocateButton(button) {
 ...
 */
export function createWhereAmIHandler({
  getPlaces,
  showUser,
  focusPlace,
  openPlace,
  ...
  return async function whereAmI() {
    ...
    try {
      ...
      try {
        ...
        // Local-first (sección 6): antes de rendirse, se intenta con la última
        // posición conocida.
        const lastKnown = getLastKnownPosition();
        if (!lastKnown) {
          const code = /** @type {any} */ (error)?.code;
          notify(ERROR_MESSAGES[/** @type {LocationErrorCode} */ (code)] || ERROR_MESSAGES.unknown);
          return;
        }
