...
(function (global) {
   ...
   */
  function sortByDistance(point, places) {
    return places
   ...
   * Base de los filtros por proximidad de la v0.3 y de las consultas PostGIS
   ...
   * @returns {NearestResult[]}
   */
  function within(point, places, radiusMeters) {
    return sortByDistance(point, places).filter((r) => r.distance <= radiusMeters);
  }

  QEA.define("geoService", { distanceMeters, formatDistance, sortByDistance, nearest, within });
})(globalThis);
